...
plot_drc_no_estimation <- function(dat_drc, Odat){
  ggplot() + geom_point(data=Odat, aes(x=A, y=Y, color=factor(L), group=1), alpha=0.5) +
    ...
    #theme(legend.position = NULL)#, legend.direction = "horizontal", legend.box = "horizontal") #+ 
    #labs(color='L:') +
    #scale_linetype_manual("",
    #                      breaks = c("truth"),
    #                      values = c("solid"))
...
plot_src <- function(dat_src_det, dat_src_sto){
  ...
  geom_line(col="cornflowerblue", linetype="solid") + 
  geom_line(aes(x=shift, y=lmtp, group=2), linetype="dashed", col="cornflowerblue") +
  geom_ribbon(aes(ymin=lmtp_low, ymax=lmtp_high, group=3), alpha=0.3, fill="cornflowerblue") +
  geom_line(data=dat_src_det, aes(x=shift, y=response, group=4), col="darkgreen") + 
  geom_line(data=dat_src_det, aes(x=shift, y=lmtp, group=5), col="darkgreen", linetype="dashed") +
  geom_ribbon(data=dat_src_det, aes(ymin=lmtp_low, ymax=lmtp_high, group=6), alpha=0.3, fill="darkgreen") +
...
plot_src_d <- function(dat_src_stody, dat_src_detdy){
  ggplot(data=dat_src_stody, aes(x=shift, y=response, group=1)) + geom_line(col="cornflowerblue") + 
    geom_line(aes(x=shift, y=lmtp, group=2), col="cornflowerblue", linetype="dashed") +
    geom_ribbon(aes(ymin=lmtp_low, ymax=lmtp_high, group=3), alpha=0.3, fill="cornflowerblue") +
    geom_line(data=dat_src_detdy, aes(x=shift, y=response, group=4), linetype=1, alpha=1, col="darkgreen") + 
    geom_line(data=dat_src_detdy, aes(x=shift, y=lmtp, group=5), linetype=2, alpha=1, col="darkgreen") +
    geom_ribbon(data=dat_src_detdy, aes(ymin=lmtp_low, ymax=lmtp_high, group=6), alpha=0.3, fill="darkgreen") +
    ggtitle("relative SRC: dynamic (only L=1)") 
}


plot_src_th <- function(dat_src_detth){
  ggplot(data=dat_src_detth, aes(x=threshold, y=response, group=1)) + geom_line(col="darkgreen") + 
  geom_line(aes(x=threshold, y=lmtp, group=2), linetype="dashed", col="darkgreen") +
  geom_ribbon(aes(ymin=lmtp_low, ymax=lmtp_high, group=3), alpha=0.3, fill="darkgreen") + 
...
plot_src_sh <- function(dat_src_detsh){
  ggplot(data=dat_src_detsh, aes(x=shift, y=response, group=1)) + geom_line(col="darkgreen") + 
  geom_line(aes(x=shift, y=lmtp, group=2), linetype="dashed", col="darkgreen") +
  geom_ribbon(aes(ymin=lmtp_low, ymax=lmtp_high, group=3), alpha=0.3, fill="darkgreen") +
...
add_double_legend <- function(plot, dat_src_sto){
  ...
  src_plot <- ggplot(data=dat_src_sto, aes(x=shift, y=response, group=1)) + 
    ...
    geom_line(aes(x=shift, y=lmtp, group=2, linetype="estimate"), col="cornflowerblue") +
    geom_ribbon(aes(ymin=lmtp_low, ymax=lmtp_high, group=3), alpha=0.3, fill="cornflowerblue") +
    geom_line(data=dat_src_sto, aes(x=shift, y=response, group=4, col="deterministic")) + 
    geom_line(data=dat_src_sto, aes(x=shift, y=lmtp, group=5), col="darkgreen", linetype="dashed") +
    geom_ribbon(data=dat_src_sto, aes(ymin=lmtp_low, ymax=lmtp_high, group=6), alpha=0.3, fill="darkgreen") +
    ggtitle("relative SRC") +
    scale_color_manual("", breaks=c('deterministic', 'stochastic'),
                       values=c('darkgreen', 'cornflowerblue')) +
...
add_single_legend <- function(plot, dat_src_detth){
  ...
  thresh_plot <- ggplot(data=dat_src_detth, aes(x=threshold, y=response, group=1)) + 
    geom_line(aes(linetype="truth"), col="darkgreen") + 
    geom_line(aes(x=threshold, y=lmtp, group=2, linetype="estimate"), col="darkgreen") +
    geom_ribbon(aes(ymin=lmtp_low, ymax=lmtp_high, group=3), alpha=0.3, fill="darkgreen") + 
    ggtitle("relative threshold RC") +
